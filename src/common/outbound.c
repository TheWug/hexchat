...
cmd_ctcp (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (*to)
	{
		...
		if (*msg)
		{
			...
			EMITX_SIGNAL (XP_TE_CTCPSEND, sess, to, msg);
...
cmd_execs (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (sess->running_exec == NULL)
	{
		EMITX_SIGNAL (XP_TE_NOCHILD, sess);
		return FALSE;
	}
...
cmd_exec (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (*cmd)
	{
		...
		if (sess->running_exec != NULL)
		{
			EMITX_SIGNAL (XP_TE_ALREADYPROCESS, sess);
			return TRUE;
		}
...
cmd_ignore (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	while (1)
	{
		if (!*word[i])
		{
			...
			{
			case 1:
				EMITX_SIGNAL (XP_TE_IGNOREADD, sess, mask);
				break;
			case 2:	/* old ignore changed */
				EMITX_SIGNAL (XP_TE_IGNORECHANGE, sess, mask);
			}
			return TRUE;
		}
		...
		if (*msg)
		{
			...
			else
			{
				...
				if (g_ascii_strcasecmp (nick, "nickserv") == 0 &&
					...
					msg = "identify ****";
				EMITX_SIGNAL (XP_TE_MSGSEND, sess, nick, msg);
			}
...
cmd_notice (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (*word[2] && *word_eol[3])
	{
		while ((split_text = split_up_text (sess, text + offset, cmd_length, split_text)))
		{
			...
			EMITX_SIGNAL (XP_TE_NOTICESEND, sess, word[2], split_text);
		...
		EMITX_SIGNAL (XP_TE_NOTICESEND, sess, word[2], text + offset);
...
cmd_notify (struct session *sess, char *tbuf, char *word[], char *word_eol[])
{
	...
	if (*word[2])
	{
		...
		while (1)
		{
			...
			{
				EMITX_SIGNAL (XP_TE_DELNOTIFY, sess, word[i]);
				return TRUE;
			}
			...
			else
			{
				...
				EMITX_SIGNAL (XP_TE_ADDNOTIFY, sess, word[i]);
			}
		}
	} else
	{
	...
	if (*mask)
	{
		...
		if (ignore_del (mask, NULL))
		{
			if (g_ascii_strcasecmp (arg, "QUIET"))
				EMITX_SIGNAL (XP_TE_IGNOREREMOVE, sess, mask);
		}
		return TRUE;
	}
	return FALSE;
