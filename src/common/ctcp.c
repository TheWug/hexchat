				 ...
				 const message_tags_data *tags_data)
{
	...
	if (!g_ascii_strncasecmp (msg, "ACTION ", 7))
	{
		...
		inbound_action (sess, sess->server, to, nick, ip, msg + 7, FALSE, id, tags_data);
		return;
	}
	...
	{
#ifdef WIN32
		snprintf (outbuf, sizeof (outbuf), "VERSION WexChat "PACKAGE_VERSION" [x%d] / %s",
					 get_cpu_arch (), get_sys_str (1));
#else
		snprintf (outbuf, sizeof (outbuf), "VERSION WexChat "PACKAGE_VERSION" / %s",
					 get_sys_str (1));
#endif
